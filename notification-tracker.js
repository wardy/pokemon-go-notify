...
module.exports = function NotificationTracker() {
  ...
  this.isEqual = function(notifiedMon, newMon) {
    return (notifiedMon.pokemon_id == newMon.pokemon_id &&
      notifiedMon.lat == newMon.lat &&
      notifiedMon.lng == newMon.lng &&
      notifiedMon.despawn == newMon.despawn);
  }

  this.hasMonBeenNotified = (pokemonReadyForNotification) => !(this.notifiedMons.length > 0 && this.notifiedMons.find(notifiedMon => this.isEqual(notifiedMon, pokemonReadyForNotification)))
